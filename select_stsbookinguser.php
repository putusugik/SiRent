<?php
	...
	if(isset ($_GET["id"])){
		$id = $_GET['id'];
		$qry = "SELECT tb_rental.`ID`, tb_dtrental.`ID_kendaraan`, tb_kendaraan.`nama_kendaraan`, tb_kendaraan.`no_plat`, DATE_FORMAT(tb_rental.`tgl_sewa`, '%d-%m-%Y %H:%i:%s') AS tgl_sewa, DATE_FORMAT(tb_rental.`tgl_kembali`, '%d-%m-%Y %H:%i:%s') AS tgl_kembali,
				...
				WHERE tb_rental.`ID_user`='$id' order by tb_rental.tgl_sewa DESC;";
