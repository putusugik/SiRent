<?php
	...
	if(isset ($_GET["id"])){
		$id = $_GET['id'];
		$qry = "SELECT tb_dtrental.`ID_kendaraan`, tb_kendaraan.`nama_kendaraan`, tb_kendaraan.`no_plat`, tb_rental.`tgl_sewa`, tb_rental.`tgl_kembali`,
				tb_profiluser.`nama_belakang`, tb_status_rental.`status_rental`
				FROM tb_dtrental
				INNER JOIN tb_rental ON tb_dtrental.`ID_rental`=tb_rental.`ID`
				INNER JOIN tb_status_rental ON tb_rental.`status`=tb_status_rental.`ID`
...
WHERE tb_rental.`status`=1 && tb_rental.`ID_toko`=$id;";
